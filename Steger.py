...
def _derivation_with_Filter(Gaussimg):
    ...
    dy = cv2.filter2D(Gaussimg, -1, z = np.array([[1, 0, -1]]))
...
def test_Steger():
    img = cv2.imread(r"binary_image.png", cv2.IMREAD_GRAYSCALE)
    print(img.shape)
    ...
    plt.axis('off')
    plt.show()
    pseudo_color_image = cv2.applyColorMap(img, cv2.COLORMAP_JET)

    print(pseudo_color_image.shape)

    img, newimage = steger.centerline(pseudo_color_image,sigmaX=1.1,sigmaY=1.1,method="Sobel")
    plt.imshow(newimage)
    plt.axis('off')
    plt.show()
...
def steger(bin_img):
    res_img = Steger(bin_img)
    pseudo_color_image = cv2.applyColorMap(bin_img, cv2.COLORMAP_JET)
    img, new_img = res_img.centerline(pseudo_color_image, sigmaX=2, sigmaY=2, method="Scharr")
